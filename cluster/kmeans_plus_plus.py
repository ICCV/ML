...
class KmeansPlusPlus():
    ...
    #计算样本集合的均值向量
    def compute_average(self,slist):
    ...
    def find_centroid(self,data,N,s):
        centroid = -1
        max_distance = 0
        for i in range(N):
            if i in s:
                continue
            for j in s:
                dis = self.compute_distance(data[i],data[j])
                if dis > max_distance:
                    centroid = i
                    max_distance = dis
        return centroid
    ...
    def clustering(self,data, k):
        ...
        #3.找出k个质心
        while len(s) < k:
            centroid = self.find_centroid(data, N, s)
            s.append(centroid)
        #4.遍历样本点,计算与质心距离并分配
        ...
        for i in range(N):
            ...
            for j in range(len(s)):
                dis = self.compute_distance(data[i],data[s[j]])
        ...
        while True:
            ...
            for i,v in zip(range(len(centroid_group_dict)),centroid_group_dict.values()):
                tmp_centroid_dict[i] = self.compute_average(v)
            for i in range(N):
                min_dis = -1
                i_centroid = -1
                for j,v in tmp_centroid_dict.iteritems():
                    dis = self.compute_distance(data[i],v)
                    if min_dis == -1 or dis<min_dis:
                        i_centroid = j
                        min_dis = dis
